...
async function getElevator(req, res) {
  const elevator = await Elevator.findOne({ elevatorId: req.params.id.toUpperCase() });
...
async function externalRequest(req, res) {
  try {
    ...
    if (!floor || !['UP', 'DOWN'].includes(direction)) {
      return res.status(400).json({ error: 'floor and direction (UP/DOWN) are required' });
    }
    if (floor < 1 || floor > NUM_FLOORS) {
      return res.status(400).json({ error: `floor must be between 1 and ${NUM_FLOORS}` });
    }

    const elevators = await Elevator.find();
    if (!elevators.length) return res.status(500).json({ error: 'No elevators configured' });
    ...
    if (elevator.direction === 'IDLE') {
      elevator.direction = floor > elevator.currentFloor ? 'UP' : floor < elevator.currentFloor ? 'DOWN' : 'IDLE';
    }
    ...
    res.json({ assignedElevator: elevator.elevatorId, estimatedArrivalTime: eta, stopsUpdated: stops });
  } catch (err) {
...
async function internalRequest(req, res) {
  try {
    ...
    if (!elevatorId || !destinationFloor) {
      return res.status(400).json({ error: 'elevatorId and destinationFloor are required' });
    }
    if (destinationFloor < 1 || destinationFloor > NUM_FLOORS) {
      return res.status(400).json({ error: `destinationFloor must be between 1 and ${NUM_FLOORS}` });
    }

    const elevator = await Elevator.findOne({ elevatorId: elevatorId.toUpperCase() });
    if (!elevator) return res.status(404).json({ error: 'Elevator not found' });
    ...
    elevator.stops = stops;
    if (elevator.direction === 'IDLE') {
      elevator.direction = destinationFloor > elevator.currentFloor ? 'UP' : 'DOWN';
    }
    elevator.status = elevator.currentFloor === destinationFloor ? 'DOOR_OPEN' : 'MOVING';
    await elevator.save();
